...
The current Taipei MRT app but with route times in real-time (to the nearest minute without needing Wi-Fi) by accomodating train waiting time, and also providing more variants of MRT routes rather than just the fastest or lowest transfer count ones (e.g. not passing certain stations or lines), alongside more requirements to satisfy my "MRT nerd" (捷運迷) self.
